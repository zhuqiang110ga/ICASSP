...
class ContextAwareTransformer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=8, shift_size=0,
        ...
        self.atten = Attention(dim=dim,num_heads=num_heads,bias=False)
        self.norm2 = LayerNorm(dim,LayerNorm_type='0')
        self.mlp = FeedForward(dim=dim,ffn_expansion_factor=mlp_ratio,bias=False)
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        x = x.flatten(2).transpose(1, 2).contiguous()  # B C H W ==> B Ph*Pw C
